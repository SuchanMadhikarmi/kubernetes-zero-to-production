---
title: Lesson 44 - Cluster Architecture and the Kubeconfig File
...
# Lesson 44 - Cluster Architecture and the Kubeconfig File
...
| Typing `kubectl` fully each time | Slow on the exam and on-call | Use the `k` alias from Lesson 43. |
...
- [Lesson 27 - CKA Exam Masterclass (Speed and Shortcut Techniques)](lesson-43-cka-exam-masterclass.md) - `k` alias and speed techniques that complement context handling.
- [Lesson 19 - ConfigMaps and Secrets](../06-configuration/lesson-20-configmaps-and-secrets.md) - namespaces and manifest anatomy.
- [Lesson 1 - The Anatomy of a Container](../01-fundamentals/lesson-01-anatomy-of-a-container.md) - the container runtime that sits behind the kubelet.

## Coming Next

Lesson 45 continues the CKA core-domain series: the control plane components (API Server, Scheduler, Controller-Manager, etcd) and how the kubelet drives them, plus the troubleshooting of a failing control-plane node.